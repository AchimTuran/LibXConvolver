...
#include "../../LXCHandles/LXC_Native/LXC_Native_types.h"
...
{
	if(!In || !fftHandle_out || Size > fftHandle_out->fftSize)
	{
		return LXC_ERR_INVALID_INPUT;
	}

	// cast pointers
	fftwf_complex *out = ((FFTWF_HANDLE*)fftHandle_out->specific_fftPlan)->x_in;
	float *in = (float*)In;
	const uint inputFramelength = fftHandle_out->maxInputFrameLength;

	if(Size == inputFramelength)
	{
		fftwf_complex *oldIn = out + fftHandle_out->maxInputFrameLength;
		for(uint ii=0; ii < fftHandle_out->fftZeros; ii++)
		{
			out[ii][0] = oldIn[ii][0];
			out[ii][1] = oldIn[ii][1];
		}
		
		out = ((FFTWF_HANDLE*)fftHandle_out->specific_fftPlan)->x_in + fftHandle_out->fftZeros;
	}
...
{
	if(!fftHandle_in || !Out || fftHandle_in->fftSize < Size)
	{
		return LXC_ERR_INVALID_INPUT;
	}
	
	// cast pointers
	fftwf_complex *in = ((FFTWF_HANDLE*)fftHandle_in->specific_fftPlan)->x_out;
	float *out = (float*)Out;
	const uint inputFramelength = fftHandle_in->maxInputFrameLength;
...
{
	if(!In1 || !In2 || !fftHandle_out || fftHandle_out->fftSize < Size)
	{
		return LXC_ERR_INVALID_INPUT;
	}
...
{
	if(!fftHandle_in || !Out1 || !Out2 || fftHandle_in->fftSize < Size)
	{
		return LXC_ERR_INVALID_INPUT;
	}
...
{
	if(!In || !fftHandle_out || fftHandle_out->fftSize < Size)
	{
		return LXC_ERR_INVALID_INPUT;
	}

	LXC_cpxFloat *in = (LXC_cpxFloat*)In;
	fftwf_complex *out = ((FFTWF_HANDLE*)fftHandle_out->specific_fftPlan)->X;
	for(uint ii=0; ii < Size; ii++)
	{
		out[ii][0] = in[ii][0];
		out[ii][1] = in[ii][1];
...
LXC_ERROR_CODE fmtc_fftwf_TO_LXCcpxFloat(LXC_FFT_PLAN *fftHandle_in, void *Out, uint Size)
{
	...
	fftwf_complex *in = ((FFTWF_HANDLE*)fftHandle_in->specific_fftPlan)->X;
...
LXC_ERROR_CODE fmtc_LXCcpxSSE3Float_TO_fftwf(void *In, LXC_FFT_PLAN *fftHandle_out, uint Size)
{
	...
	Size = Size*2;
	float *in = (float*)In;
	float *out = (float*)((FFTWF_HANDLE*)fftHandle_out->specific_fftPlan)->X;
...
LXC_ERROR_CODE fmtc_fftwf_TO_LXCcpxSSE3Float(LXC_FFT_PLAN *fftHandle_in, void *Out, uint Size)
{
	...
	Size = Size*2;
	float *in = (float*)((FFTWF_HANDLE*)fftHandle_in->specific_fftPlan)->X;
	float *out = (float*)Out;
	for(uint ii=0; ii < Size; ii+=4)
	{
		//out[ii][0] = in[ii][0];
