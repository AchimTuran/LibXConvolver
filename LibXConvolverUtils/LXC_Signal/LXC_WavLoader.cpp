...
unsigned int LXC_CWavLoader::openWavFile(string WavFile, LXC_WavStruct *WavHandle)
{
	if(WavFile.empty())
	{
		throw LXC_EXCEPTION_COUT_HANDLER("WavFile was empty!");
	}

	if(WavHandle == NULL)
	{
		throw LXC_EXCEPTION_COUT_HANDLER("WavHandle == NULL! Invalid Input!");
	}
    ...
    return 0;
		//throw LXC_EXCEPTION_COUT_HANDLER("Wav file " + WavFile + " not found!" + " libsndfile error: " + sndErrorStr);
	}
	...
	{
		deleteWav(WavHandle);
		throw LXC_EXCEPTION_COUT_HANDLER("WavFile " + WavFile + "has < 1 channels! Invalid input file!");
	}
	...
	{
		deleteWav(WavHandle);
		throw LXC_EXCEPTION_COUT_HANDLER("Samplerate from the filter file was < 1! Invalid input filter file!");
	}
	...
	{
		deleteWav(WavHandle);
		throw LXC_EXCEPTION_COUT_HANDLER("Filter file " + WavFile + " was empty! No filter taps were loaded!");
	}
	...
	{
		deleteWav(WavHandle);
		throw LXC_EXCEPTION_COUT_HANDLER("Could not create buffer for filter! Not enough free memory?!?");
	}
	...
	{
		throw LXC_EXCEPTION_COUT_HANDLER("calculated filter size was greate than the filter in the file! Is the filter file corrupted?");
	}

	return WavHandle->maxSamples;
}

void LXC_CWavLoader::deleteWav(LXC_WavStruct *WavHandle)
{
	if(WavHandle)
	{
		if(WavHandle->samples)
...
void LXC_CWavLoader::reorderChannels(LXC_WavStruct *WavHandle)
{
	if(!WavHandle)
	{
		throw LXC_EXCEPTION_COUT_HANDLER("WavHandle == NULL! Invalid input!");
	...
	if(WavHandle->samples)
	{
		...
		if(!tempFilter)
		{
			throw LXC_EXCEPTION_COUT_HANDLER("Could not allocate enough dynamic memory! Is there not enough free memory?");
		}
...
bool LXC_CWavLoader::checkWavStruct(LXC_WavStruct *WavHandle)
{
	if(!WavHandle)
	{
		throw LXC_EXCEPTION_COUT_HANDLER("WavHandle == NULL! Invalid input!");
	...
	{
		deleteWav(WavHandle);
		throw LXC_EXCEPTION_COUT_HANDLER("Samplerate from the filter file was <= 0! Invalid input filter file!");
	}
	...
	{
		deleteWav(WavHandle);
		throw LXC_EXCEPTION_COUT_HANDLER("There were no filter taps!");
	}
